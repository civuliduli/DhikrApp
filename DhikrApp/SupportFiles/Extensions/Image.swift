//
//  Image.swift
//  DhikrApp
//
//  Created by Abdulla Civuli on 10.11.24.
//

import UIKit

extension UIImage {
    // Home Icons
    static var homeIcon: UIImage = UIImage(systemName: "house")!
    static var homeIconFilled: UIImage = UIImage(systemName: "house.fill")!
    
    // Bookmark Icons
    static var bookMarkIcon: UIImage = UIImage(systemName: "bookmark")!
    static var bookMarkIconFilled: UIImage = UIImage(systemName: "bookmark.fill")!
    
    // Tasbih Icons
    static var tasbihIcon: UIImage = UIImage(systemName: "arrow.trianglehead.counterclockwise")!
    
    // Settings Icons
    static var settingsIcon: UIImage = UIImage(systemName: "gearshape")!
    static var settingsIconFilled: UIImage = UIImage(systemName: "gearshape.fill")!
    
    // Like icon
    static var heartIcon: UIImage = UIImage(systemName: "heart")!
    static var heartIconFilled: UIImage = UIImage(systemName: "heart.fill")!
    
    // Bookmark icon
    static var streakBookIcon: UIImage = UIImage(systemName: "book.fill")!
    
    // Refresh icon
    static var refreshIcon: UIImage = UIImage(systemName: "arrow.clockwise")!
    
    // Share icon
    static var shareIcon: UIImage = UIImage(systemName: "square.and.arrow.up")!
    
    // Time icon
    static var timeIcon: UIImage = UIImage(systemName: "clock.arrow.trianglehead.counterclockwise.rotate.90")!
    
    static var checkMarkIcon: UIImage = UIImage(systemName: "checkmark")!
    static var moonIcon: UIImage = UIImage(systemName: "moon")!
    static var sunIcon: UIImage = UIImage(systemName: "sun.max")!
    static var cpuIcon: UIImage = UIImage(systemName: "cpu")!
    
    
}
